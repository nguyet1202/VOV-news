 ...
 * @package news-jack
 */
if ( ! function_exists( 'newsjack_enqueue_styles' ) ) :
	/**
	 * @since 0.1
	 */
	function newsjack_enqueue_styles() {
		wp_enqueue_style( 'newsup-style-parent', get_template_directory_uri() . '/style.css' );
		wp_enqueue_style( 'newsjack-style', get_stylesheet_directory_uri() . '/style.css', array( 'newsup-style-parent' ), '1.0' );
		wp_enqueue_style('bootstrap', get_template_directory_uri() . '/css/bootstrap.css');
		wp_dequeue_style( 'newsup-default',get_template_directory_uri() .'/css/colors/default.css');
		wp_enqueue_style( 'newsjack-default-css', get_stylesheet_directory_uri()."/css/colors/default.css" );
		if(is_rtl()){
...
add_action( 'wp_enqueue_scripts', 'newsjack_enqueue_styles', 9999 );


add_action( 'customize_register', 'newsjack_customizer_rid_values', 1000 );
function newsjack_customizer_rid_values($wp_customize) {
...
function newsjack_theme_setup() {

//Load text domain for translation-ready
load_theme_textdomain('newsjack', get_stylesheet_directory() . '/languages');
...
add_action( 'after_setup_theme', 'newsjack_theme_setup' );
 ...
 */
function newsjack_widgets_init() {
	

	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Canvas Section', 'news-jack'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Left Sidebar Section', 'news-jack'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Right Sidebar Section', 'news-jack'),
...
add_action( 'widgets_init', 'newsjack_widgets_init' );


function newsjack_remove_some_widgets(){
// Unregister Frontpage sidebar
unregister_sidebar( 'front-page-sidebar' );
}
add_action( 'widgets_init', 'newsjack_remove_some_widgets', 11 );

function newsjack_menu(){ ?>
<script>
...
<?php }
add_action( 'wp_footer', 'newsjack_menu' );